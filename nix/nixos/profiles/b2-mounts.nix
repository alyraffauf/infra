{
  ...
  cfg = config.myNixOs.profile.b2Mounts;
...
in {
  options.myNixOs.profile.b2Mounts = {
    enable = lib.mkEnableOption "Backblaze B2 mounts";
  ...
  config = lib.mkIf config.myNixOs.profile.b2Mounts.enable {
