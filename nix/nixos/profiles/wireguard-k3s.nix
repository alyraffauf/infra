{
  ...
  cfg = config.myNixOs.profile.wireguardK3s;
  nodes = {
    jubilife = {
      address = "10.254.0.1";
...
in {
  options.myNixOs.profile.wireguardK3s.enable = lib.mkEnableOption "the dedicated k3s WireGuard mesh";
  ...
  config = lib.mkIf cfg.enable {
    ...
    networking = {
      firewall = {
        ...
        trustedInterfaces = lib.mkBefore ["wg-k3s"];
      };
      hosts =
