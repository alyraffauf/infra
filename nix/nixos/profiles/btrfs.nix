...
in {
  ...
  config = lib.mkIf config.myNixOs.profile.btrfs.enable {
    ...
    services = lib.mkIf (btrfsFSDevices != []) {
      beesd.filesystems = lib.mkIf config.myNixOs.profile.btrfs.deduplicate beesdConfig;
